...
object CustomGetOffsetShell {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val offsetConfigOpt = parser.accepts("offset.config", s"Consumer config properties file.")
      .withRequiredArg
    ...
    val config =
      if (options.has(offsetConfigOpt))
        Utils.loadProps(options.valueOf(offsetConfigOpt))
      else new Properties
